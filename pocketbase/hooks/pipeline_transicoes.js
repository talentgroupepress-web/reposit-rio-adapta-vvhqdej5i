...
// onRecordValidate roda em todo $app.save(); recusa = throw + log via $app.logger().
...
onRecordValidate((e) => {
  ...
  if (recordId && recordId !== '') {
    try {
      ...
      old = null // não existe ainda → create
    }
  }
  ...
  // Helper inline de recusa (tudo inline — sem top-level)
  const recusar = (motivo, extra) => {
    $app.logger().warn(
      'transicao_recusada',
      Object.assign(
        {
  ...
  // A partir de prospect, registros não terminais exigem os 3 campos (CA-1-07).
  if (NAO_TERMINAIS_ACAO.includes(estadoAtual)) {
    ...
    if (faltando.length > 0) {
      recusar(
        'registro nao terminal a partir de prospect exige responsavel, proxima_acao e prazo (CA-1-07 / emenda E1)',
  ...
  if (estadoAnterior === 'suspect' && estadoAtual === 'prospect') {
    ...
    if (!prazo || prazo.trim() === '') faltando.push('prazo')
    if (faltando.length > 0) {
      recusar('suspect -> prospect exige responsavel, proxima_acao e prazo (emenda E1)', {
        faltando: faltando.join(','),
      })
    }
  }
  ...
  // 3 condições: ICP + necessidade real + contato válido.
  // Limitação técnica: não há campo de ICP nesta fase — validar partes possíveis
  // (evidência de necessidade, contato válido, serviço definido). ICP fica pendência.
  ...
  if (estadoAnterior === 'proposta' && estadoAtual === 'vaga_aberta') {
    ...
    // Conversão comercial = data preenchida no aceite
  ...
  // Ao entrar em vaga_aberta com evidência de primeira demanda válida, data_conquista é obrigatória.
  // data_conquista só pode ser preenchida em vaga_aberta ou ganho.
  if (
    dataConquista &&
    dataConquista.trim() !== '' &&
